...
test('Contact form validates required fields', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Email sent!' })).not.toBeVisible();
...
test('Contact form handles long input values', async ({ page }) => {
  ...
  const longEmail = 'very.long.email.address.test.123456789@really.long.domain.name.example.com';
